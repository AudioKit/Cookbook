        ...
        VStack  {
            ...
            ValueSlider(value: self.$parameter, in: self.range)
                .valueSliderStyle(HorizontalValueSliderStyle(
                    ...
                    thumbInteractiveSize: CGSize(width: 40, height: 40)))
        }
    }
}
...
struct ParameterSlider_Previews: PreviewProvider {
    ...
    static var previews: some View {
        Group {
            ParameterSlider(text: "Text", parameter: $param1, range: 0...1).previewLayout(PreviewLayout.fixed(width: 400, height: 200))
                .padding()
                .background(Color(.systemBackground))
                .environment(\.colorScheme, .dark)
                .previewDisplayName("Dark Mode")
